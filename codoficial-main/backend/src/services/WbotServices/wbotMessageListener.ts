...
import UpdateTicketService, { TicketData } from "../TicketServices/UpdateTicketService";
...
import ShowQueuesByUserService from "../QueueService/ShowQueuesByUserService";
...
export const verifyContact = async (
  props:IVerifyContactProps
): Promise<Contact> => {
  const { wbot, companyId, msgContact, newContact } = props;
  ...
  const contactData = {
    name: newContact ? newContact.name : msgContact?.name || msgContact.id.replace(/\D/g, ""),
    number: newContact ? newContact.number : msgContact.id.replace(/\D/g, ""),
    profilePicUrl,
    isGroup: newContact ? false : msgContact.id.includes("g.us"),
    companyId,
  };
...
const handleMessage = async (
  ...
  try {
    ...
    if (isGroup) {
      ...
      groupContact = await verifyContact({ msgContact: msgGroupContact, wbot, companyId });
    }
    ...
    const [createdTicket, ticket] = await FindOrCreateTicketService({ contactId: contact.id, whatsappId: wbot.id!, unreadMessages, companyId, groupContactId: groupContact?.id });
    ...
    const [created, ticketTraking] = await FindOrCreateATicketTrakingService({
      ticketId: ticket.id,
...
const verifyCampaignAndHandleTicket = async (
  wbot: Session,
  message: proto.IWebMessageInfo,
  companyId: number
) => {
  ...
  const isCampaignTicket = ticket.status === 'campaign';
  const isCampaignMessage = /\u200c/.test(body);
  let ticketUpdate:TicketData = {
    status: "campaign",
    userId: ticket.userId,
    queueId: ticket.queueId,
  };
  if (isCampaignMessage) {
    await UpdateTicketService({ ticketData: ticketUpdate, ticketId: ticket.id, companyId: ticket.companyId });
  } else if (isCampaignTicket) {
    ...
    wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
      ...
      messages.forEach(async (message: proto.IWebMessageInfo) => {
        ...
        if (!messageExists) {
          ...
          await verifyCampaignAndHandleTicket(wbot, message, companyId);
        }
      });
    });
